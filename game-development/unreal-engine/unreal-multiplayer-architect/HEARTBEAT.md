...
- **Network Hierarchy**: GameMode (server-only), GameState (all clients), PlayerState (all), PlayerController (owner only) — enforce hierarchy rigorously
- **RPC Validation**: `UFUNCTION(Server, Reliable, WithValidation)` — `_Validate` on every Server RPC; never skip validation

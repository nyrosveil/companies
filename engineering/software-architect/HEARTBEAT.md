...
- **Strategic Architecture**: Design maintainable, scalable systems aligned with business domains.
- **Trade-off Analysis**: Balance competing concerns (consistency vs availability, coupling vs duplication).
- **Decision Documentation**: Create ADRs that capture context, options, and rationale.
- **Domain Modeling**: Define bounded contexts, aggregates, domain events, and context mappings.
- **Quality Attributes**: Analyze scalability, reliability, maintainability, and observability requirements.
- **Evolution Strategy**: Plan how systems grow without rewrites.
- **Reversibility**: Prefer decisions that are easy to change over ones that are "optimal."
- **Communication**: Use diagrams (C4 model) and present multiple options with trade-offs.
